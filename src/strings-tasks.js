...
function formatTime(minutes, seconds) {
  ...
  if (minTime >= 0 && minTime < 10) {
    minTime = minTime.toString().padStart(2, '0');
  }
  if (secTime >= 0 && secTime < 10) {
    secTime = secTime.toString().padStart(2, '0');
  }
...
function isPalindrome(str) {
  const arrStr = str.toLowerCase().trim().replaceAll(/\W/g, '');
  return arrStr === arrStr.split('').reverse().join('');
}
...
function findLongestWord(sentence) {
  ...
  for (let i = 0; i < newSentence.length; i += 1) {
    if (maxLenth < newSentence[i].length) {
      maxLenth = newSentence[i].length;
      longWord = newSentence[i];
    }
  }
  return longWord;
}
...
function reverseWords(str) {
  const arrStr = str.split(' ');
  const result = arrStr.map((item) => item.split('').reverse().join(''));
  return result.join(' ');
}
...
function encodeToRot13(str) {
  ...
  const arrStr = str.split('');
  for (let i = 0; i < arrStr.length; i += 1) {
    if (upperAlfabet.includes(arrStr[i])) {
      const index = upperAlfabet.indexOf(arrStr[i]);
      newStr += upperAlfabet[(index + 13) % 26];
    } else if (lowerAlfabet.includes(arrStr[i])) {
      const index = lowerAlfabet.indexOf(arrStr[i]);
      newStr += lowerAlfabet[(index + 13) % 26];
    } else {
      newStr += arrStr[i];
    }
  }
